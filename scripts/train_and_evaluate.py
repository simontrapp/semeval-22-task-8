...
compute_similarities('data/processed/train', 'data/semeval-2022_task8_train-data_batch.csv', TRAINING_DATA_CSV_PATH, sbert_models, universal_sentence_encoder_model)

# STEP 2: train random forest regressor
train_random_forest(TRAINING_DATA_CSV_PATH, 'models/random_forest_test.joblib', True)   # train and evaluate on test set (create visualization/data for paper)
train_random_forest(TRAINING_DATA_CSV_PATH, RANDOM_FOREST_FILE, False)                  # use the whole data for training the random forest

# STEP 3: process evaluation data
compute_similarities('data/processed/eval', 'data/semeval-2022_task8_eval_data_202201.csv', EVAL_DATA_CSV_PATH, sbert_models, universal_sentence_encoder_model, True)
