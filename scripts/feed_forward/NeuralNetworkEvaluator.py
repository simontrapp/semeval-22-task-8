...
class NeuralNetworkEvaluator(SentenceEvaluator):
    """
    ...
    def __init__(self, sentences1: List[str], sentences2: List[str], scores: List[float], network: nn.Module,
    ...
    def __call__(self, model, output_path: str = None, epoch: int = -1, steps: int = -1) -> float:
        ...
        embeddings2 = model.encode(self.sentences2, batch_size=self.batch_size,
                                   show_progress_bar=self.show_progress_bar, convert_to_tensor=True)
        labels = torch.Tensor(self.scores)

        pred = self.network(torch.cat((embeddings1, embeddings2), dim=1))
        tp = torch.sum(torch.argmax(pred, dim=1) == torch.argmax(labels, dim=1))
        accuracy = tp / len(labels)
        print(metrics.classification_report(torch.argmax(labels,dim=1).detach(), torch.argmax(pred,dim=1).detach()))
        # print(f"accuracy: {accuracy}")
        print(f"\npredicted labels {torch.unique(torch.argmax(pred, dim=1), return_counts=True)}")
        print(f"actual labels {torch.unique(torch.argmax(labels, dim=1), return_counts=True)}")
        return accuracy
