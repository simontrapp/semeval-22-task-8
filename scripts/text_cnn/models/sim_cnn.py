...
class SimCnn(nn.Module):
    def __init__(self, loss_fn, device="cuda"):
        ...
        self.in_5 = SimCnnPart(kernel_size=5, device=device)
    ...
    def forward(self, x):
        ...
        x_4 = self.in_4(x)
        x_5 = self.in_5(x)
        x = torch.cat([x_2, x_3, x_4, x_5], dim=1)
        return self.out(x)
...
class SimCnnPart(nn.Module):
    def __init__(self, kernel_size,  device="cuda"):
        ...
        self.final_network = nn.Sequential(
            CnnBlock(in_channels=1, out_channels=16, expand=32, kernel_size=kernel_size),
            CnnBlock(in_channels=16, out_channels=32, expand=64, kernel_size=kernel_size),
            ...
            CnnBlock(in_channels=256, out_channels=512, expand=1024, kernel_size=kernel_size),
            MaxOverTimePooling(),
        )
