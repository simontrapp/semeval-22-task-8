...
class SentenceDataset(Dataset):
    ...
    def __getitem__(self, idx):
        e1 = self.encoder(self.sentences_1[idx])
        e1 = np.concatenate((e1, np.zeros((max - e1.shape[0], e1.shape[1]))))
        e2 = self.encoder(self.sentences_2[idx])
        e2 = np.concatenate((e2, np.zeros((max - e2.shape[0], e2.shape[1]))))
