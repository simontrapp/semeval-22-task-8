...
def create_universal_sentence_encoder_embeddings(model, input_sentences: list):
    ...
    try:
        for index, row in sentence_pairs.iterrows():
            ...
            if os.path.exists(first_json_path) and os.path.exists(second_json_path):
                # read data
                sentences_1 = process_json_to_sentences(first_json_path)
                sentences_2 = process_json_to_sentences(second_json_path)
                # score similarities
